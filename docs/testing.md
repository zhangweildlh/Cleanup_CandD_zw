---
title: 测试历史
description: v0.5.0 及之前版本的 Pester 3.4.0 冒烟测试运行方式、45 用例覆盖矩阵（全功能/全场景/全边界）与 PS 5.1 环境约束。v0.5.1 起仓库不再自带测试套件，本文档转为历史存档。
related:
  - architecture.md
  - configuration.md
...
> **v0.5.1 起，本仓库不再自带 Pester 测试套件**（`tests/` 目录已移除）。
> 本文档保留为**历史存档**，记录 v0.5.0 及之前版本的测试运行方式、覆盖矩阵与 PS 5.1 陷阱，
> 便于追溯回归基线。当前版本的正确性验证改由**外部审计**承担（`powershell-audit-regression` 技能：语法/AST 静态审计 + 动态行为复现）。

v0.5.0 及之前，本仓库用 **Pester 3.4.0**（PowerShell 5.1 自带版本）做黑盒冒烟测试：用受控 `.ini` / `.csv` 驱动脚本，断言其产物（CSV / 标准输出），**不依赖脚本内部函数导出**，避免改动业务脚本。

## 一、运行方式（历史，v0.5.0 及之前）
