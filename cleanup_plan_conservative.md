...
## 一、数据来源
- D:\System\UserTemp\winapp2_sample_expanded.csv
...
| 待确认（需确认，非安全根） | 5 | 0.00 | 默认不删，需用户决定 |
...
> 共 5 个文件 / 0.00 MB，默认不删除。
> 以下按所在目录聚合展示；如需逐文件绝对路径，请运行脚本时加 `-FullList`，或查阅同目录完整清单 CSV。

| 所在目录 | 文件数 | 总体积(MB) | 主要分类 |
| --- | --- | --- | --- |
| D:\System\UserTemp\zw_demo_app | 2 | 0.00 | Applications |
| D:\System\UserTemp\zw_exclude_app | 1 | 0.00 | Applications |
| C:\Users\Administrator\AppData\Local\Microsoft\Edge\User Data\TestProfile | 1 | 0.00 | Google Chrome |
| C:\Windows\Temp | 1 | 0.00 | Applications |

> 注：仅展示文件数前 500 的目录；完整 5 项见同目录清单 CSV。
